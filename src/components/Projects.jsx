    ...
    >
      <div className="bg-[#0f172a] border border-white/8 rounded-3xl w-full max-w-2xl max-h-[88vh] overflow-y-auto p-8 relative">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <p className="text-indigo-400 text-xs tracking-widest uppercase font-bold mb-1">
              Project
            </p>
        ...
        {project.image && (
          <img
            src={project.image}
            alt={project.title}
            className="w-full rounded-2xl mb-6 border border-white/10 object-cover"
          />
        )}

        {/* Description */}
        <h3 className="text-indigo-400 text-xs font-bold tracking-widest uppercase mb-2">
          Description
        </h3>
        ...
        {/* Features */}
        <h3 className="text-indigo-400 text-xs font-bold tracking-widest uppercase mb-3">
          Key Features
        </h3>
        <ul className="space-y-2 mb-6">
          {project.features.map((f) => (
            <li
              key={f}
              className="flex gap-3 text-slate-300 text-sm border-b border-white/5 pb-2"
            >
    ...
    <section id="projects" className="py-28 bg-slate-950/60">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <p className="text-indigo-400 text-xs font-bold tracking-[0.15em] uppercase mb-3">
            My Work
          </p>
          ...
          <p className="text-slate-400 max-w-md mx-auto text-base">
            A selection of projects built with the MERN stack, crafted with care
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-7">
          {projects.map((p) => (
            ...
            >
              {/* Thumb */}
              <div
                className={`h-48 bg-gradient-to-br ${p.gradient} flex flex-col items-center justify-center gap-3 relative overflow-hidden`}
              >
                {p.image ? (
                  <img
                    src={p.image}
              ...
              <div className="p-6 flex flex-col flex-1">
                <div className="flex flex-wrap gap-2 mb-4">
                  {p.tech.slice(0, 3).map((t) => (
                    <span
                      key={t}
                      className="bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs px-2.5 py-1 rounded-lg"
                    >
                      {t}
                    </span>
                  ))}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-9 h-9 rounded-xl border border-white/10 hover:border-indigo-400/40 flex items-center justify-center text-slate-400 hover:text-indigo-400 transition-all text-base"
                    >
                      🌐
                    </a>
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-9 h-9 rounded-xl border border-white/10 hover:border-indigo-400/40 flex items-center justify-center text-slate-400 hover:text-indigo-400 transition-all"
                    >
                      <svg
                        width="15"
                        height="15"
